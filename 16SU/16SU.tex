...
\Task{Check that reading in fits is comparable to png}
...
\DoneMiniTask{Change the piece of code from imread to getdata. {\bf Actually, changing to the astropy.io.fits.  Should be similar to getdata, but returns a 300,300 numpy array of floats.}}
...
\MiniTask{Look over git log}
\MiniTask{Map out changes that still need to be made before getting my Y-M relations}
\Diss
...
\MiniTask{Produce intermediate images after each preprocessing step - e.g. see example in http://scikit-image.org/docs/dev/auto_examples/applications/plot_rank_filters.html}
